			...
			if(isNotValide){
				...
				bcrypt.genSalt(10, function(err, salt) {
	    			bcrypt.hash(newUser.password, salt, function(err, hash) {
	        			...
	        			fs.writeFile('./json_test.json', JSON.stringify(data,null,2), 'utf-8', function(err) {
							if(err){
...
router.post('/login',
  ...
  function(req, res){
  	console.log(req.user.username+ "DDDDDDDDDDDDDDDDDD");
  	res.redirect('/');
...
router.get('/logout', function (req, res) {
	...
	req.flash('success_msg', 'Tu est déconnecté');
